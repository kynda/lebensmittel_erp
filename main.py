...
@app.post("/kunden/neu")
def add_kunde(name: str = Form(...), adresse: str = Form(""), typ: str = Form("B2C"), db: Session = Depends(get_db)):
...
def add_charge(produkt_id: int = Form(...), datum: str = Form(...), menge: float = Form(...), db: Session = Depends(get_db)):
